...
def main(args, init_distributed=False):
    ...
    discriminator_k.to(torch.device("cuda:0"))
    # discriminator_k = SimpleCNN(len(task.dicts['ar'])).cuda()
    # doptimizer_k = torch.optim.Adam(discriminator_k.parameters(),lr=0.0002)

    params = list(discriminator_k.parameters())+list(model.models['ar-ke'].encoder.parameters())+ \
        list(model.models['ke-ke'].encoder.parameters())
    ...
    dmodel = {"ar":dmodel_a,"ke":dmodel_k,'model_c':dmodel_c, 'nepochs':0, 'nsteps':0}
    ...
    # while lr > args.min_lr  and trainer.get_num_updates() < max_update:
        ...
        dmodel['nepochs'] = epoch_itr.epoch
...
def calc_gradient_penalty(netD, batch_size, real_data, fake_data):
    LAMBDA = 10
    use_cuda = True
...
def train(args, epch, trainer, task, epoch_itr, dmodel, model, criterion, nsteps):
    ...
    fout = open("{}/discriminator.log".format(args.d_path),"w")
    

    # print(">>>>",len(itr))

    dmodel['max_size'] = len(itr)
    ...
    for i, samples in enumerate(progress, start=epoch_itr.iterations_in_epoch):
        ...
        for i, sample in enumerate(samples):
            ...
            # dweight = min(1e-2, 1e-2*(nsteps/100000))
            dweight = 1.0
            ...
            for src_pair in args.lang_pairs:
                ...
                if src_pair == 'ke-ke' or  src_pair == 'ar-ke':
                    ...
                    discriminator, doptimizer, = dmodel['ar']['model'], dmodel['ar']['optimizer'], \
                    ...
                    #T x B x C
                    # seq_out = encoder_out['encoder_out'][1].clone().detach()

                    # seq_out = encoder_out['encoder_out'][1]
                    # seq_out = seq_out.permute(1,0,2)


                    # print(seq_out.size())

                    seq_out = encoder_out['encoder_out'][0].transpose(1,0)
                    # seq_out = torch.mean(seq_out, 1, keepdim=True)

                    # cnn_input = torch.cat([seq_out_1,seq_out], dim=1)
                    # seq_out = seq_out.permute(1,0,2)

                    d_lim = 300 if seq_out.size(1) > 300 else seq_out.size(1)
                    cnn_input = torch.zeros(seq_out.size(0),300,seq_out.size(2))
                    ...
                    one = torch.ones(rewards.size()).cuda()
                    mone = -1 * one
 

                    if src_pair == "ke-ke":
                        rewards.backward(one)
                    else:
                        rewards.backward(mone)
                ...
                if  src_pair == 'ar-ke':
                    ...
                    # seq_out_1 = encoder_out['encoder_out'][1]
                    # seq_out_1 = seq_out_1.permute(1,0,2)
                    # seq_out = encoder_out['encoder_out'][0].transpose(1,0)
                    # seq_out = torch.mean(seq_out, 1, keepdim=True)
                    # # print(">>",seq_out.size())
                ...
                def get_encoder_output(src_pair):
                    src_tokens = sample[src_pair]['net_input']['src_tokens']
                    src_lengths = sample[src_pair]['net_input']['src_lengths']
                    encoder_out = model.models[src_pair].encoder(src_tokens, src_lengths)
                    seq_out = encoder_out['encoder_out'][0].transpose(1,0)
                ...
                hypo_ke = get_encoder_output('ke-ke')
                hypo_ar = get_encoder_output('ar-ke')


                discriminator.zero_grad()
                penalty = calc_gradient_penalty(discriminator,hypo_ke.size(0),hypo_ke,hypo_ar)
