...
def main(args, init_distributed=False):
    ...
    bceloss_k = torch.nn.BCEWithLogitsLoss(reduction='mean')
    ...
    dmodel_k ={'model': discriminator_k, 'loss': bceloss_k, 'optimizer': doptimizer_k}


    discriminator_a = SimpleCNN(len(task.dicts['ar'])).cuda()
    doptimizer_a = torch.optim.Adam(discriminator_a.parameters(),lr=0.0002)
    bceloss_a = torch.nn.BCEWithLogitsLoss(reduction='mean')
    ...
    dmodel_a ={'model': discriminator_a, 'loss': bceloss_a, 'optimizer': doptimizer_a}



    dmodel = {"ar":dmodel_a,"ke":dmodel_k}
    ...
    # while lr > args.min_lr  and trainer.get_num_updates() < max_update:
        ...
        train(args, epoch_itr.epoch, trainer, task, epoch_itr, dmodel)
...
def train(args, epch, trainer, task, epoch_itr, dmodel):
    ...
    for i, samples in enumerate(progress, start=epoch_itr.iterations_in_epoch):
        ...
        if src_pair == 'ke-ke' or  src_pair == 'ar-ke':
                ...
                hypos = self.inference_step(generator,[model.models[src_pair]],sample[src_pair])
                

                # inp = Variable(net_output[0].clone().detach())
                rewards = discriminator(hypos)               
                rewards = rewards.to(dev)
                target = torch.ones(rewards.size()) if src_pair == 'ke-ke' else torch.zeros(rewards.size())
        ...
        if src_pair == 'ar-ar' or  src_pair == 'ke-ar':
                ...
                rewards = discriminator(hypos)               
                rewards = rewards.to(dev)
                target = torch.ones(rewards.size()) if src_pair == 'ar-ar' else torch.zeros(rewards.size())
