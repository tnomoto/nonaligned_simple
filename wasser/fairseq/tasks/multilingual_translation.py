...
class MultilingualTranslationTask(FairseqTask):
    ...
    def train_step(self, epch, step, sample, model, criterion, optimizer, dmodel, generator,ignore_grad=False):
        ...
        # accept = 1.0 - kl_anneal_function("logistic", dmodel['nepochs'], 10, 2)

        roll = np.random.random_sample()

        # print(f"{dmodel['max_size']}")
        # cur_point = dmodel['max_size'] * dmodel['nepochs'] + dmodel['nsteps']
        # mid_point = dmodel['max_size'] * 12 * 1./10.

        # mid_point = dmodel['max_size'] // 2.0
        # cur_point = dmodel['nsteps']
        # accept = 1.0 - kl_anneal_function("logistic", cur_point, 1., mid_point)

        # print(f"cur_point: {cur_point} mid_point: {mid_point} accept: {accept}: roll: {roll}")
