...
class Trainer(object):
    ...
    def __init__(self, args, task, model, criterion,dummy_batch=None, oom_batch=None):
        ...
        # import sys
        # print ("task len {}".format(len(task.dicts['ar'])))
        # sys.exit(0)
        # self.discriminator = SimpleCNN(len(task.dicts['ar'])).cuda()
        # self.doptimizer = torch.optim.Adam(self.discriminator.parameters(),lr=0.0002)
    ...
    def train_step(self, epch, count, generator, samples, dstate, dummy_batch=False, raise_oom=False):
        ...
        for i, sample in enumerate(samples):
            ...
            try:
                ...
                # loss, sample_size, logging_output = self.task.train_step(epch, count,
                #     sample, self.model, self.criterion,  self.optimizer, 
                #     dstate['model'], dstate['optimizer'], dstate['loss'], generator,
                #     ignore_grad
                # )
        ...
        # discriminator_state_a = {'epoch': epch,
        #                         'model': dstate['ar']['model'].state_dict(),
        #                         'optimizer': dstate['ar']['optimizer'].state_dict(),
        #                         }


        discriminator_state_k = {'epoch': epch,
                                'model': dstate['ke']['model'].state_dict(),
                                'optimizer': dstate['ke']['optimizer'].state_dict(),
                                }
        ...
        # return  logging_output, (discriminator_state_a, discriminator_state_k)
        return  logging_output, (discriminator_state_k)


    def valid_step(self, sample, dmodel, raise_oom=False):
        ...
        with torch.no_grad():
            ...
            try:
                _loss, sample_size, logging_output = self.task.valid_step(
                    sample, self.model, self.criterion, dmodel
                )
            except RuntimeError as e:
                if 'out of memory' in str(e) and not raise_oom:
                    ...
                    return self.valid_step(sample, dmodel,raise_oom=True)
                else:
                    raise e
