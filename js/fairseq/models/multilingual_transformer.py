    ...
    def build_model(cls, args, task):
        ...
        def get_decoder_cnn(lang):
            if lang not in lang_decoders:
                ...
                lang_decoders[lang] = TransformerDecoderCNN(args, task.dicts[lang], decoder_embed_tokens)
            return lang_decoders[lang]

        # shared encoders/decoders (if applicable)
        shared_encoder, shared_decoder = None, None
        if args.share_encoders:
            print(f"enc  {src_langs[0]}")
            shared_encoder = get_encoder(src_langs[0])
        if args.share_decoders:
            # shared_decoder = get_decoder(tgt_langs[0])
            shared_decoder_d = {}
            for k in tgt_langs:
                if k not in shared_decoder_d:
                    shared_decoder_d[k]=get_decoder(k)


        encoders, decoders = OrderedDict(), OrderedDict()
        for lang_pair, src, tgt in zip(task.model_lang_pairs, src_langs, tgt_langs):
            print(f"pair {lang_pair} src {src} tgt {tgt}")
            ...
            decoders[lang_pair] = shared_decoder_d[tgt] if shared_decoder is not None else get_decoder(tgt)
...
@register_model_architecture('multilingual_transformer', 'multilingual_transformer_pg')
def multilingual_transformer_iwslt_de_en(args):
