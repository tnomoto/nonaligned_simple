...
class LabelSmoothedCrossEntropyCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, lang_pair, dmodel, reduce=True):
        ...
        # if dmodel is None:
        #     return net_output[0]
        # discriminator_k = dmodel['ke']['model']
        # discriminator_a = dmodel['ar']['model']


        # # discriminator.to(torch.device("cuda:0"))
        # seq_out = encoder_out['encoder_out']
        ...
        # # hypo = hypo.to(torch.device("cuda:0"))
        # rewards_k = discriminator_k(hypo)
        # rewards_a = discriminator_a(hypo)

        # rewards = rewards_a + rewards_k
        # if lang_pair == "ar-ke":
        #     loss_a, nll_loss_a = self.reinforce_loss(model, net_output, sample, rewards_a, reduce=True)
        #     loss_b, nll_loss_b = self.reinforce_loss(model, net_output, sample, 1.0 - rewards_k, reduce=True)
        #     loss = loss_a + loss_b
        #     nll_loss = nll_loss_a + nll_loss_b
        # else:
